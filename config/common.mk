...
ifeq ($(CYANIDE_BUILDTYPE), RELEASE)
    ifndef TARGET_VENDOR_RELEASE_BUILD_ID
        CYANIDE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(CYANIDE_BUILD)
    else
        ifeq ($(TARGET_BUILD_VARIANT),user)
            CYANIDE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)-$(CYANIDE_BUILD)
        else
            CYANIDE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR).$(PRODUCT_VERSION_MAINTENANCE)$(PRODUCT_VERSION_DEVICE_SPECIFIC)-$(CYANIDE_BUILD)
        endif
    endif
    ...
    ifeq ($(PRODUCT_VERSION_MINOR),0)
        CYANIDE_VERSION := $(PRODUCT_VERSION_MAJOR)-$(shell date -u +%Y%m%d)-$(CYANIDE_BUILDTYPE)$(CYANIDE_EXTRAVERSION)-$(CYANIDE_BUILD)
    else
        CYANIDE_VERSION := $(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(shell date -u +%Y%m%d)-$(CYANIDE_BUILDTYPE)$(CYANIDE_EXTRAVERSION)-$(CYANIDE_BUILD)
    endif
endif

PRODUCT_PROPERTY_OVERRIDES += \
  ro.cyanide.version=6.0 \
...
ifneq ($(PRODUCT_DEFAULT_DEV_CERTIFICATE),build/target/product/security/testkey)
  ifneq ($(CYANIDE_BUILDTYPE), UNOFFICIAL)
    ...
    CYANIDE_DISPLAY_VERSION=$(PRODUCT_VERSION_MAJOR).$(PRODUCT_VERSION_MINOR)-$(TARGET_VENDOR_RELEASE_BUILD_ID)
  endif
endif
endif
